import React from 'react'

const Fashion = () => {
  return (
    <>
    <div className="banner d-flex justify-content-center">
    <img className="image-banner" src="./WomenFashion.png" alt="" />
    </div>
     <div className='container'>
        <section className="cards mt-5">
          <article className="horizontal card">
            <img className="card-image" src="./ada_fashion2.jpg" alt="" />
              <div className="card-content">
                <div className="card-type">women fashion</div>
                  <div className="card-title">Ada Fashion
        </div>
                <div className="card-excerpt">text
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
    </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
  </section>
  </div>

  <div className="banner d-flex justify-content-center">
    <img className="image-banner" src="./Menfashion.png" alt="" />
    </div>
    <div className='container'>




        <section className="cards mt-5">
          <article className="horizontal card">
            <img className="card-image" src="./ada_fashion2.jpg" alt="" />
              <div className="card-content">
                <div className="card-type">article</div>
                  <div className="card-title">Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">text
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
    </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    
  </section>

  
  </div>
  <div className="banner d-flex justify-content-center">
    <img className="image-banner" src="./kidsfashion.png" alt="" />
    </div>
    <div className='container'>
        <section className="cards mt-5">
          <article className="horizontal card">
            <img className="card-image" src="./ada_fashion2.jpg" alt="" />
              <div className="card-content">
                <div className="card-type">article</div>
                  <div className="card-title">Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">text
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
    </article>
                  <article className="vertical card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
              <div className="card-content">
            <div className="card-type">article</div>
          <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
                <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
        </div>
        </article>
                  <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="vertical card">
      <img className="card-image" src="./matahari_fashion.jpg" alt="" />
      <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
    <article className="horizontal card">
                <img className="card-image" src="./matahari_fashion.jpg" alt="" />
            <div className="card-content">
        <div className="card-type">article</div>
        <div className="card-title">
          Test Post: Electric Boogaloo
        </div>
        <div className="card-excerpt">
          percobaan untujk wesb percobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesbpercobaan untujk wesb
        </div>
      </div>
    </article>
  </section>
  </div>

    </>
  )
}

export default Fashion
